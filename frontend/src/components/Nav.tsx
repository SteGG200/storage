...
export default function Nav({ path }: NavProps) {
	...
	const rightIndexCollapse =
		items.length - maxItemsToDisplay + leftIndexCollapse;
					...
					{items.map((item, index) => {
						if (
							(items.length > maxItemsToDisplay &&
								(index < leftIndexCollapse || index >= rightIndexCollapse)) ||
							items.length <= maxItemsToDisplay
						) {
							return (
								<Fragment key={index}>
														...
														if (
															...
															return (
																<DropdownMenuItem
																	...
																	asChild
																>
																	<Link href={getHref(index)}>{item}</Link>
																</DropdownMenuItem>
															);
														}
													})}
