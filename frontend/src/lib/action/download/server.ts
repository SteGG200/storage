...
export const getSizeOfFile = async (path: string, filename: string) => {
	// Get total size of file
	const token = await getToken();
	const response = await fetch(
		`${process.env.NEXT_PUBLIC_SERVER_URL}/download/${path}/${filename}`,
		{
			method: 'HEAD',
			headers: {
...
export const downloadChunk = async (
	...
	await fs.appendFile(
		`${process.env.TMP_DIR}/${hashString}_${filename}`,
		buffer
	);
};
