...
import { Dropdown, DropdownTrigger, DropdownMenu, DropdownItem, useDisclosure, Modal, ModalHeader, ModalBody, ModalContent, ModalFooter, Input, Button } from "@nextui-org/react"
							...
							{ actionType === "upload" ?
								<>
									<div className="space-y-10">
										<Input type="text" isRequired label="File name" name="name" placeholder="Enter your file's name" isDisabled={isPending} isInvalid={filenameError} onChange={() => setFilenameError(false)} errorMessage="Please enter a valid file's name"/>
										<Input type="file" isRequired label="File" name="file" isDisabled={isPending} isInvalid={fileError} onChange={() => setFileError(false)} errorMessage="Please upload a valid file"/>
									</div>
								...
								:
								<>
									<Input type="text" isRequired label="Folder name" name="name" placeholder="Enter your folder's name" isDisabled={isPending} isInvalid={foldernameError} onChange={() => setFolderNameError(false)} errorMessage="Please enter a valid folder's name"/>
									{ error && <p className="text-customRed">{error}</p> }
									<Button type="submit" color="primary" isLoading={isPending}>Submit</Button>
								</>
							}
